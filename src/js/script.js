...
player.on("fullscreenchange", (event) => {
  if (player.fullscreen.active) {
    // Remove the size restriction on the video when entering fullscreen mode
    ...
    // Restore the original size restriction on the video when exiting fullscreen mode
    document.querySelector(".plyr__video-wrapper video").style.maxWidth =
      "1080px";
    document.querySelector(".plyr__video-wrapper video").style.width = "100%";
...
window.javascriptLocales = {
  copyMessage:
    "Just wanted to let you know that Masuzu is always with you! Even inside your clipboard! We are everywhere and you should really join us!\n" +
    "Also yes, any website can access your clipboard however they want, isn't that creepy?\n" +
    "Regardless, Satania is the BEST WAIFU, and you should agree! http://masuzu.moe/",
...
//   if (guild) {
//     document.getElementById("discord-number").innerText =
...
//   if (subreddit) {
//     document.getElementById("reddit-number").innerText =
//       subreddit.data.subscribers.toLocaleString(document.documentElement.lang);
